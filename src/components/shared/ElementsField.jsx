...
const ElementsTable = styled.div`
    ...
    height: calc(100% - ${({$ratio}) => $ratio * 116}px);
...
export const ElementsField = ({onElementClick, openedElements, shownElements = elements, page = 1}) => {
    ...
    const getIsOpen = (element) => (page === 1 || openedElements.includes(element?.id))

    const handleClick = (element) => {
        if (!getIsOpen(element)) return;
    ...
    return (
        <Wrapper $ratio={ratio}>
            <SvgWrapper width="100%" height="100%" viewBox="0 0 380 471" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin meet">
            ...
            <ElementsTable $ratio={ratio}>
                {gridElements.map((element) => (
                    <ElementCell key={element.id} $grid={element.grid} onClick={() => handleClick(element)}>
                        {getIsOpen(element) ? (
